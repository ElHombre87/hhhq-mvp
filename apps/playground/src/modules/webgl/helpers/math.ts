/**
 * 
 * @param args x, y normalized centered coordinate of mouse position
 */
export function mouseVectorLength(...args: [mouseX: number, mouseY: number]) {
 const [x, y] = args.map(Math.abs);
 const length = Math.sqrt(x*x + y*y - 2 * x * y * Math.cos(90));
 return length;
}
